...
# end
function true_kras_loss(ut::Array{T,1}, pf::AbstractArray, pv::AbstractArray, m::KrasNDDE) where {T<:Real}
    xt = ut[m.fmask]
    yt = ut[m.vmask]
    v, vx = forwardgrad(m.re_v(pv), yt)
    b = vx[1:m.data_dim]'*m.re_f(pf)(xt)
    for i in 1:length(m.vlags)
        b += vx[i*m.data_dim+1:(i+1)*m.data_dim]'*m.re_f(pf)(ut[m.fmask .+ i*m.data_dim])
    end
    return relu(b + m.α * v[1]) / (v[1] + 1e-3)
end
function kras_loss(ut::Array{T,1}, pf::AbstractArray, pv::AbstractArray, m::KrasNDDE) where {T<:Real}
    xt = ut[m.fmask]
    yt = ut[m.vmask]
    v, vx = forwardgrad(m.re_v(pv), yt)
    b = vx[1:m.data_dim]'*m.re_f(pf)(xt)
...
function kras_loss(ut::Array{T,2}, pf::AbstractArray, pv::AbstractArray, m::KrasNDDE) where {T<:Real}
    xt = ut[m.fmask, :]
    yt = ut[m.vmask, :]
    v, vx = forwardgrad_batched(m.re_v(pv), yt)
    b = dot.(eachcol(vx[1:m.data_dim,:]), eachcol(m.re_f(pf)(xt)))
    for i in 1:length(m.vlags)
