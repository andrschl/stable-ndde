...
# constructor
StableDynamics(data_dim;nfdelays=0, nvdelays=0, alpha=0.001, p=1.002, act=C2_relu) = begin
...
function krasovskii_loss(m::StableDynamicsKrasovksii, xt::AbstractArray, yt::AbstractArray, p::AbstractArray, q::AbstractArray)
    data_dim = m.data_dim
    if yt == zero(yt)
        return 0.0
    end
    ...
    b = 0.0
    for i in 1:m.nvdelays
        b += vx[i*data_dim+1:(i+1)*data_dim]'*yt[i*data_dim+1:(i+1)*data_dim]
    end
    return relu(vx'*m.f_hat(xt) + m.alpha * v[1]) / (v[1] + 1e-3)
