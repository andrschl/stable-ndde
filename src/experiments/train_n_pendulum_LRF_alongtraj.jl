...
function sample_RKHS_h0s(init_t, data_dim; k0="RBF", rng=Random.GLOBAL_RNG,
            α_lim=3.0, α_min_rad=0.8, scaling_lims=(1.0, 10.0), magnitude_lims=(0.01, 1.0))
...
@time begin
    ...
    for (lr, iter) in lr_schedule
        ...
        # combined train step
        raz_stable_ndde_train_step!(batch_h0(nothing, batch_t[1]), batch_u, batch_h0, pf, pv, batch_t, model, optf, optv, iter, lyap_loader)
