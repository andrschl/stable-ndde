...
project_name = "cartpole_LRF_alongtraj"
runname = "seed_"*string(seed)
configname = "/"*string(config["α"])
...
if !config["server"]
    pl = plot()
    for (j, traj) in enumerate(df_model.trajs[1:1])
            for i in 1:data_dim
                # scatter!(pl, traj[1], hcat(traj[2]...)[i,:])
                plot!(pl, -1:0.01:tspan_lyap[2],t-> traj[3](t)[i])
            end
...
for (j, traj) in enumerate(df_model.trajs)
    ...
    CSV.write(logpath*"test_pred_LQR"* string(j) *".csv", pred, header = true)
    ...
    for (lr, iter) in lr_schedule
        ...
        if !config["server"] && (iter % 1==0)
            ...
            for (j, traj) in enumerate(df_model.trajs[1:4])
                for i in 1:data_dim
                    # scatter!(pl, traj[1], hcat(traj[2]...)[i,:])
                    plot!(pl_train, traj[1][1]:0.01:tspan_lyap[2],t-> traj[3](t)[i])
                end
            end
            display(pl_train)
            # if iter % 1 == 0
